const backoff = {
  exponential(attempt, delay) {
    return Math.floor(Math.random() * (2 ** attempt) * delay);
  },
...
Backoff.prototype.backoff = function prototypeBackoff() {
  this.attempts += 1;
...
class WebSocketClient {
   ...
   * @ignore
   */
  onCloseCallback(e, ...args) {
    if (!this.isReconnect && this.listeners.onclose) {
      this.listeners.onclose.apply(null, args);
    }
  ...
  onErrorCallback(...args) {
    if (this.listeners.onerror) {
      this.listeners.onerror.apply(null, args);
    }
  }
  ...
  onMessageCallback(...args) {
    if (this.listeners.onmessage) {
      this.listeners.onmessage.apply(null, args);
    }
  }
  ...
  onOpenCallback(...args) {
    if (this.listeners.onopen) {
      this.listeners.onopen.apply(null, args);
    }

    if (this.isReconnect && this.listeners.onreconnect) {
      this.listeners.onreconnect.apply(null, args);
    }
  ...
  close(code, reason) {
    ...
    this.ws.close(typeof code === 'undefined' ? 1000 : code, reason);
  }
