(function () {
        ...
        methods: {
            ...
            getDisabledReason: function(mode) {
                if (mode === ControlMode.SINGLE_CONTROLLER) {
                    ...
                    let gp = navigator.getGamepads();
                    for(let i = 0; i < gp.length; i++) {
                        if (gp[i] && checkVidPid(gp[i].id, '57e', '2006')) left = true;
                        else if (gp[i] && checkVidPid(gp[i].id, '57e', '2007')) right = true;
                    }
    ...
    const JoyconStreamRendererWebRTC = {
        mixins: [JoyconSprites],
        props: ['endpoint'],
        data: function () {
            return {
